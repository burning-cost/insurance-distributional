"""
Run the NeuralGaussianMixture test suite on Databricks serverless compute.

Embeds the project zip as base64 in the notebook, installs torch (CPU),
and runs tests/test_neural_gmm.py. No external storage needed.
"""
import os
import sys
import time
...
WORKSPACE_PATH = "/Workspace/insurance-distributional-neural-tests"
NOTEBOOK_PATH = f"{WORKSPACE_PATH}/run_neural_gmm_pytest"
...
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    for pattern in ["src/**/*.py", "tests/test_neural_gmm.py", "pyproject.toml", "README.md"]:
...
NOTEBOOK_CONTENT = (
    ...
    "# COMMAND ----------\n"
    "# Install torch (CPU) plus other deps — quiet to keep output manageable\n"
    "import subprocess, sys\n"
    "r = subprocess.run(\n"
    "    [sys.executable, '-m', 'pip', 'install', '--quiet',\n"
    "     'catboost', 'polars', 'scipy', 'numpy', 'pytest',\n"
    "     'torch', '--index-url', 'https://download.pytorch.org/whl/cpu'],\n"
    "    capture_output=True, text=True\n"
    ")\n"
    "print('pip torch exit:', r.returncode)\n"
    ...
    "\n"
    "extract_dir = Path(tempfile.mkdtemp(prefix='ins-dist-neural-'))\n"
    ...
    "     str(extract_dir / 'tests' / 'test_neural_gmm.py'),\n"
    "     '-v', '--tb=short', '-x'],\n"
...
# ---------------------------------------------------------------------------
# Step 3: Submit as a serverless one-shot job run
# ---------------------------------------------------------------------------
print("Submitting serverless job run...")

run = w.jobs.submit(
    run_name="insurance-distributional-neural-gmm-pytest",
    tasks=[
        jobs.SubmitTask(
            task_key="run_neural_gmm_pytest",
...
# ---------------------------------------------------------------------------
print("Polling (takes ~5-10 min for serverless startup + torch install + tests)...")
start = time.time()
while True:
